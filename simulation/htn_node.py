...
class HTNNode:
    ...
    def __init__(self, name='', node_type=PRIMITIVE, action=None, agent=Agent.UNASSIGNED, parent=None, state=INCOMPLETE):
    ...
    def convert_to_decision(self):
        if self.node_type == HTNNode.PRIMITIVE and self.agent == Agent.UNASSIGNED:
            if lookup_cost(self.action, Agent.ROBOT) >= 0 and lookup_cost(self.action, Agent.HUMAN) >=0:
                n1 = HTNNode(name=self.name + '-r', node_type=HTNNode.PRIMITIVE, parent=self, action=self.action,
                             agent=Agent.ROBOT)
                n2 = HTNNode(name=self.name + '-h', node_type=HTNNode.PRIMITIVE, parent=self, action=self.action,
                             agent=Agent.HUMAN)
                self.set_children([n1, n2])
                self.probabilities = [.5, .5]
                self.node_type = HTNNode.DECISION
                self.action = None
            elif lookup_cost(self.action, Agent.ROBOT) >= 0:
                self.agent = Agent.ROBOT
            elif lookup_cost(self.action, Agent.HUMAN) >= 0:
                self.agent = Agent.HUMAN
