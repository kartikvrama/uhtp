...
if __name__ == "__main__":
    ...
    if not rospy.is_shutdown():
        input('Press enter to end current run')
        end1 = time.time()
        ...
        rospy.sleep(1)
        start2 = time.time()
        action_timer_2, logmsgs = ROSHTN.run(robot_behavior=ROBOT_BHV, debugfile=debugfile)
        ...
        # Log robot failures
        if logmsgs != '':
            exfile.write('Run 2 failures: {}\n'.format(logmsgs))
            # Terminate if segmentation error
            if logmsgs != 'gripper':
                raise RuntimeError

        # Run 2 and final summary
        exfile.write('RUN2: Time taken in seconds: {:7.2f}\n'.format(end2-start2))
        exfile.write('Total time taken in seconds: {:7.2f}'.format(end1-start1 + end2-start2))
        ...
        if ROBOT_BHV == 'adaptive':
            exfile.write('\nRun 2 action timer\n')
            for key in action_timer_2.keys():
                exfile.write('{}:- {:7.2f}\n'.format(key, action_timer_2[key]))
            exfile.write('\n')

            action_timer.update(action_timer_2)
            with open(os.path.join(logdir_path, 'adaptive_action_timer.p'), 'wb') as pickf:
